...
class ScoreResultTest < ActiveSupport::TestCase
  ...
  # end
  def setup
    @origin_score = rand(9999999)
    @base_score = rand(9999999)
  ...
  test "caculate the result score automatically" do
    @score_result = ScoreResult.create(user_id: @user.id, score_base_id: @score_base.id, score_origin: @origin_score)
    assert_in_delta @score_result.score_result, @origin_score * 1.0 / @base_score, 0.1
  end
